...
However, the true \pzpdf\ is in general not accessible unless the photometry is in fact drawn from the true \pz\ likelihoods, which is not how most existing datasets have been constructed.  Instead, data is often taken from actual observations, or generated from mock catalogs derived from N-body simulations of the Universe (as is the case in this study).  In these cases there is no ``true PDF'' for an individual object, and most measures of PDF fidelity will need to rely on ensemble properties of the similar objects, or the entire sample.  We mention a potential project addressing this issue in \S\,\ref{sec:futureexperiments}.
%a mock catalogue generation procedure that has not yet appeared in the literature and was not performed for this work.
...
The quantile-quantile (QQ) plot serves as a graphical visualization for comparing two distributions, where the quantiles of one distribution are plotted against the quantiles of the other distribution, providing an easy way to qualitatively assess the consistency between an estimated distribution and a true distribution.
