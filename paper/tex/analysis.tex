...
\boldblue{While a discrete approximation to the true $p(z,d)$ is possible by sampling the local neighbourhood of parameter space in large datasets with methods like a nearest neighbor or conditional density estimate, we do not make an explicit computation of such an approximate distribution.  Rather, we note that the CMNN and FlexZBoost algorithms are specific implementations of such algorithms which can be used as examples of such approximations in the absence of knowledge of the true $p(z,d)$.}
