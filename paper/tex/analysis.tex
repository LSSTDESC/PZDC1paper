...
\begin{equation}
  \label{eq:cdf}
  \rm{CDF}(q) \equiv \int_{-\infty}^{q} p(z) dz,
\end{equation}
of a \pzpdf, which is used as the basis for several of our metrics.
...
\subsection{Conditional density estimation loss}
\label{sec:CDE_loss}
%(Alex Malz, Ann Lee)
...
We use the QQ plot to visualize the relationship between $\hat{N}^{H}(z)$ and $\tilde{N}(z)$.
