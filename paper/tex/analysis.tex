...
Galaxies have redshifts $z$ and photometric data $d$ drawn from a joint probability space $p(z, d)$ in nature that is not known to us.
If it were known, however, each galaxy $i$ would have a \textit{true posterior \pzpdf}\ $p(z \vert d_{i})$ as well as a true likelihood $p(d \vert z_{i})$.
...
In this study, we isolate the effect of differences in prior information $I_{H}$ specific to each method by using a single training set $I_{D}^{ML}$ for all machine learning-based codes and a single template library $I_{D}^{T}$ for all template-based codes.
These sets of prior information are carefully constructed to be representative and complete, so we have $I_{D} \equiv I_{D}^{ML} \equiv I_{D}^{T}$ for every method $H$.
Thus, we are saying
\begin{equation}
...
Some scientific applications of \pzpdf s necessitate knowledge of each individual galaxy's \pzpdf, for example galaxy intrinsic alignment studies, strong lensing foreground shear prediction, and photometric supernova classification, while others require only knowledge of the ensemble redshift distribution, $N(z)$.
Due to the paucity of principled techniques for using and validating individual galaxy \pzpdf s in the literature, there have been few alternatives to the common practice of reducing \pzpdf s to point estimates in prior comparisons of \pz\ code performance.
Though the outmoded practice should not be encouraged, we also calculate traditional metrics based on the most common point estimators derived from \pzpdf s in Appendix~\ref{sec:pointmetrics}.
...
The exact implementation of the stacked estimator $\hat{N}^{H}(z)$ depends on the parametrization of the \pzpdf s, which may differ across codes and can affect the precision of the estimator \citep{Malz:qp}.
