...
In Section~\ref{sec:newcodes}, we interpret the performance of the genuine \pzpdf\ codes in terms of concrete improvements that should be pursued by the code developers.
...
The \trainz\ estimator, which assigns every galaxy a \pzpdf\ equal to $N(z)$ of the training set as described in Section~\ref{sec:trainz}, is introduced as an experimental control or null test to demonstrate this point via \textit{reductio ad absurdum}.
...
\subsection{Evolution of \pzpdf\ codes}
\label{sec:newcodes}

Trends in the metrics shared among codes indicate avenues for improvement of each approach.

Section~\ref{sec:pitqq} identifies over- and under-estimation of the widths of \pzpdf s by certain codes.
We suggest that the smoothing parameter present in \bpz, \cmnn, \delight, \eazy, \gpz, \skynet, and \tpz\ be chosen more carefully to improve their performance in the QQ plots and PIT histograms of
...
\subsection{Extended experimental design}
