...
In addition to packages cited in the text, analyses performed in this paper used the following software packages: \code{Numpy} and \code{Scipy} \citep{numpyscipy}, \code{Matplotlib} \citep{matplotlib}, \code{Seaborn} \citep{seaborn}, \code{minFunc} \citep{minfunc}, \code{pySkyNet} \citep{pyskynet}, and \code{photUtils} from the \lsst\ simulations package \citep{lsstphotutils}.
